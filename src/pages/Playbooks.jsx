...
function FlowStep({ number, label, detail, color = '#818cf8', last = false }) {
  return (
    <div style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
        <div style={{
          ...
          color, fontWeight: 700, fontSize: '0.85rem', fontFamily: 'var(--font-mono)',
        }}>{number}</div>
        {!last && <div style={{ width: '2px', height: '40px', background: `${color}44`, marginTop: '4px' }} />}
      </div>
      <div style={{ paddingTop: '6px', paddingBottom: last ? 0 : '24px' }}>
        <div style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: '0.9rem' }}>{label}</div>
        {detail && <div style={{ color: 'var(--text-secondary)', fontSize: '0.82rem', marginTop: '4px', lineHeight: '1.6' }}>{detail}</div>}
      </div>
    </div>
  )
}
...
export default function Playbooks() {
  ...
  const sections = [
    { key: 'structure', label: 'Play Structure' },
    { key: 'handlers', label: 'Handlers' },
    { key: 'tags', label: 'Tags' },
    { key: 'import-include', label: 'Import vs Include' },
    { key: 'error-handling', label: 'Error Handling' },
    { key: 'execution-control', label: 'Execution Control' },
  ...
  return (
    <div className="page-container fade-in">
      <PageHeader
        ...
        icon="▶"
        description="Master the full anatomy of Ansible plays and playbooks: task structure, handlers, tags, import/include, block/rescue/always error handling, and execution control."
        tags={['Play', 'Tasks', 'Handlers', 'Tags', 'Block', 'Rescue', 'Serial', 'delegate_to']}
      />
      ...
      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
        {sections.map(s => (
          <button key={s.key} onClick={() => setActiveSection(s.key)} style={{
            ...
            border: `1px solid ${activeSection === s.key ? 'var(--accent-primary)' : 'var(--border-subtle)'}`,
            background: activeSection === s.key ? 'var(--accent-primary-subtle)' : 'var(--bg-card)',
            color: activeSection === s.key ? 'var(--accent-primary)' : 'var(--text-muted)',
      ...
      {activeSection === 'structure' && (
        <>
          <h2 className="section-title">Anatomy of a Playbook</h2>
          <p>
            A <strong>playbook</strong> is a YAML file containing one or more <strong>plays</strong>.
            Each play maps a set of <strong>tasks</strong> to a group of managed hosts.
            Tasks execute in order — Ansible runs them sequentially on each host by default.
          </p>
          ...
          <div style={{
            ...
            <h3 style={{ color: 'var(--accent-primary)', marginBottom: '1rem', fontSize: '0.9rem' }}>
              Playbook Execution Flow
            </h3>
            <FlowStep number="1" label="Inventory parsing" detail="Ansible resolves which hosts match the 'hosts' pattern. Groups, host_vars, group_vars are loaded." color="#38bdf8" />
            <FlowStep number="2" label="pre_tasks" detail="Tasks that run before roles and regular tasks. Handlers triggered here fire immediately." color="#818cf8" />
            <FlowStep number="3" label="roles" detail="Roles execute in order listed. Each role's tasks/main.yml runs." color="#a78bfa" />
            <FlowStep number="4" label="tasks" detail="Regular play tasks run sequentially." color="#818cf8" />
            <FlowStep number="5" label="post_tasks" detail="Tasks that run after all other tasks. Useful for verification." color="#818cf8" />
            <FlowStep number="6" label="Handlers flush" detail="All notified handlers run at the end of the play (in order they were defined, not notified)." color="#34d399" last />
          </div>
    ...
    - name: Install PostgreSQL
      ansible.builtin.dnf:
        ...
        state: present`}
          ...
          <h3 style={{ color: 'var(--text-primary)', marginTop: '1.5rem' }}>Task-Level Keywords</h3>
          <CodeBlock
            language="yaml"
            filename="examples/task-anatomy.yml"
            code={`tasks:
  - name: A fully annotated task
    ...
    until: install_result.rc == 0`}
      ...
      {activeSection === 'handlers' && (
        <>
          <h2 className="section-title">Handlers</h2>
          <p>
            Handlers are tasks that only run when <strong>notified</strong> by another task.
            They are ideal for actions that should happen <em>only when something changed</em>,
            like restarting a service after a config file is updated.
          </p>

          <InfoBox type="key">
            Handlers run <strong>once at the end of the play</strong>, regardless of how many
            tasks notify them. They run in the <em>order they are defined</em>, not the order they
            were notified. If a play fails mid-way, pending handlers do NOT run (unless
            <code> --force-handlers</code> is passed).
          </InfoBox>

          <CodeBlock
            language="yaml"
            filename="examples/handlers.yml"
            code={`---
- name: Handler examples
  ...
  handlers:
    ...
    - name: Post-restart health check
      ...
      # Note: this runs AFTER 'Restart nginx' completes`}
          />

          <InfoBox type="exam">
            Common exam scenario: A config file is updated — the handler should restart the service.
            Make sure the <code>notify</code> value in the task <em>exactly matches</em> the handler's
            <code> name</code> (or <code>listen</code>) — it's case-sensitive!
          </InfoBox>
        </>
      )}
      ...
      {activeSection === 'tags' && (
        <>
          ...
          <p>
            Tags let you selectively run or skip parts of a playbook without editing it.
            Useful for running only the "install" phase, skipping "cleanup", or targeting
            specific components.
          </p>

          <CodeBlock
            language="yaml"
            filename="examples/tags-demo.yml"
            code={`---
- name: Tagged playbook example
  ...
  tasks:
    ...
    - name: This task always runs (ignores --tags)
      ...
      tags:
        - always   # 'always' tag: runs even when --tags is specified`}
          />

          <CodeBlock
            language="bash"
            title="Running playbooks with tags"
            code={`# Run only tasks tagged 'install' and 'packages'
...
ansible-playbook site.yml --list-tags`}
          ...
          <InfoBox type="tip">
            <strong>Tag strategy for the exam:</strong> Use <code>--tags</code> to quickly
            re-run just the broken section without running the entire playbook again.
            <code>--list-tasks</code> shows you exactly what will execute before committing.
          </InfoBox>
        </>
      )}

      {/* ── IMPORT vs INCLUDE ─────────────────────────────────── */}
      {activeSection === 'import-include' && (
        <>
          <h2 className="section-title">Import vs Include</h2>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', margin: '1rem 0' }}>
            {[
              {
                title: 'import_tasks / import_playbook',
                subtitle: 'Static — processed at parse time',
                color: '#818cf8',
                points: [
                  'Processed before the play runs',
                  'Tags on the import apply to ALL imported tasks',
                  'Can\'t use variables for the filename',
                  'Handlers from imported files are available play-wide',
                  'Loops don\'t work on import_tasks',
                ],
              },
              {
                title: 'include_tasks / include_playbook',
                subtitle: 'Dynamic — processed at runtime',
                color: '#34d399',
                points: [
                  'File is loaded when the task runs',
                  'Tags on include don\'t apply to included tasks',
                  'Filename can be a variable: "{{ env }}.yml"',
                  'Can be used inside loops',
                  'ansible --list-tasks won\'t show included tasks',
                ],
              },
            ].map(card => (
              <div key={card.title} style={{
                ...
                <div style={{ color: card.color, fontWeight: 700, marginBottom: '4px' }}>{card.title}</div>
                <div style={{ color: 'var(--text-muted)', fontSize: '0.78rem', marginBottom: '1rem' }}>{card.subtitle}</div>
                <ul style={{ margin: 0, paddingLeft: '1.2rem', color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
                  {card.points.map((p, i) => <li key={i} style={{ marginBottom: '6px' }}>{p}</li>)}
                </ul>
              </div>
            ))}
          </div>
          ...
          <CodeBlock
            language="yaml"
            filename="examples/import-include.yml"
            code={`---
# ── IMPORT_PLAYBOOK ────────────────────────────────────────────
...
#     state: present`}
      ...
      {activeSection === 'error-handling' && (
        <>
          ...
          <p>
            By default, Ansible stops executing on a host when any task fails. Error handling
            gives you fine-grained control over what counts as a failure and what to do when
            things go wrong.
          </p>

          <CodeBlock
            language="yaml"
            filename="examples/error-handling.yml"
            code={`---
- name: Error handling patterns
  ...
  tasks:
    ...
    # max_fail_percentage: 30  (in play definition)`}
          />

          <InfoBox type="warning">
            <strong>Rescue tasks don't clear "failed" status by default.</strong> If the rescue
            block completes successfully, the host continues as if it hadn't failed. But if you want
            to mark the play as failed even after rescue, add an explicit
            <code> ansible.builtin.fail</code> task inside the rescue block.
          </InfoBox>
        </>
      )}

      {/* ── EXECUTION CONTROL ─────────────────────────────────── */}
      {activeSection === 'execution-control' && (
        <>
          <h2 className="section-title">Execution Control</h2>
          <p>
            Control <em>how many hosts</em> run tasks simultaneously, implement rolling updates,
            and manage host ordering.
          </p>

          <CodeBlock
            language="yaml"
            filename="examples/execution-control.yml"
            code={`---
    ...
    - ansible.builtin.debug:
      ...
      {activeSection === 'delegation' && (
        <>
          ...
          <p>
            <code>delegate_to</code> runs a task on a <em>different host</em> than the one the
            play is targeting. All the target host's variables are still available — only the
            execution location changes. Essential for load balancer management, DNS updates,
            and centralized logging.
          </p>

          <CodeBlock
            language="yaml"
            filename="examples/delegation.yml"
            code={`---
  ...
  tasks:
    ...
    - name: Log to a local file
      ...
      # Equivalent to: delegate_to: localhost`}
          ...
          <InfoBox type="exam">
            The exam commonly tests <code>delegate_to: localhost</code> for tasks that should
            run on the <strong>control node</strong> (like downloading files, waiting for hosts,
            or making API calls from the Ansible host itself).
          </InfoBox>
        </>
      )}
    </div>
