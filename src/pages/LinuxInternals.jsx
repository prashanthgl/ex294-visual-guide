...
export default function LinuxInternals() {
  const [activeSection, setActiveSection] = useState('Kernel Architecture')
  ...
  return (
    <div className="page-container fade-in">
      <PageHeader
        ...
        icon="🐧"
        description="Deep dive into Linux kernel architecture, process lifecycle, filesystem hierarchy, storage, networking, SELinux, and systemd — everything you need to understand WHY Ansible tasks work the way they do."
        tags={['Kernel', 'Processes', 'SELinux', 'LVM', 'Systemd', 'Networking']}
      />
      ...
      {/* ===== KERNEL ARCHITECTURE ===== */}
      {activeSection === 'Kernel Architecture' && (
        <div>
          <h2 className="section-title">Linux Kernel Architecture</h2>
          <p>
            The Linux kernel is a <strong>monolithic kernel</strong> — meaning it runs entirely in
            kernel space (ring 0, privileged mode). User programs cannot directly access hardware;
            they must ask the kernel through <em>system calls</em>. This is the fundamental security
            boundary of the entire operating system.
          </p>
...
`}</pre>
          ...
          <h3 className="subsection-title">Virtual File System (VFS)</h3>
          <p>
            The VFS is an abstraction layer that allows the kernel to support many different
            filesystems (ext4, xfs, tmpfs, proc, sysfs) through a single interface. When
            you call <code>open("/etc/passwd")</code>, the VFS determines which filesystem driver
            to use, then delegates to it. This is why <code>/proc/1/status</code> and
            <code>/etc/fstab</code> look like files even though one is virtual and one is on disk.
          </p>

          <h3 className="subsection-title">System Calls — The Security Gateway</h3>
          <p>
            Every privileged operation (file I/O, process creation, network socket) requires
            a system call. The kernel validates permissions, contexts (SELinux), and resource
            limits before executing. This is why <code>strace</code> is so powerful — it shows
            every system call a process makes.
          </p>
          <CodeBlock
            language="bash"
            title="Tracing system calls"
            code={`# Trace all system calls made by a command
strace -c ls /etc
...
sysctl -p                          # apply from file`}
          ...
          <h3 className="subsection-title">Kernel Modules</h3>
          <p>
            Drivers and filesystem support are loaded as modules — dynamically loadable code that
            extends the kernel without rebooting. When you add an XFS filesystem, the <code>xfs</code> module
            must be loaded.
          </p>
          <CodeBlock
            language="bash"
            title="Kernel module management"
            code={`lsmod                          # list loaded modules
...
# Module parameters
modinfo -p e1000               # show parameters for e1000 NIC driver`}
          />

          <InfoBox type="key" title="Why This Matters for Ansible">
            <p>
              When Ansible uses the <code>sysctl</code> module or modifies <code>/etc/sysctl.conf</code>,
              it's working at the kernel parameter level. When it manages <code>firewalld</code>, it's
              configuring the netfilter kernel subsystem. Understanding these layers helps you
              debug why changes persist (or don't) after reboot.
            </p>
          </InfoBox>
        </div>
      )}
      ...
      {activeSection === 'Process Management' && (
        <div>
          <h2 className="section-title">Process Lifecycle & Management</h2>
          <p>
            Every running program is a <strong>process</strong> — an instance of a program in
            execution. The kernel manages processes in a data structure called the
            <em>process table</em>. Each process gets a unique PID (Process ID).
          </p>

          <h3 className="subsection-title">Process States</h3>
          <div className="diagram-container">
            <pre>{`
...
`}</pre>
          ...
          <CodeBlock
            language="bash"
            title="Process inspection tools"
            code={`# See all processes with state
...
fuser /var/log/messages   # simpler: PIDs using a file`}
          />

          <h3 className="subsection-title">The /proc Filesystem — Live Kernel Data</h3>
          <p>
            <code>/proc</code> is a <em>virtual filesystem</em> — its files don't exist on disk.
            They're generated by the kernel on demand. This is where the kernel exposes all its
            internal state, and it's what tools like <code>ps</code>, <code>top</code>, and
            Ansible facts use internally.
          </p>
          <CodeBlock
            language="bash"
            title="Exploring /proc"
            code={`# Every running process has a directory: /proc/<PID>/
...
test -d /proc/1234 && echo "running" || echo "not running"`}
          />

          <h3 className="subsection-title">Signals</h3>
          <p>
            Signals are the IPC (inter-process communication) mechanism for notifying processes
            about events. When you press <kbd>Ctrl+C</kbd>, the terminal sends SIGINT to the
            foreground process. Ansible's <code>command</code> and <code>shell</code> modules
            receive signals when plays are interrupted.
          </p>
          <CodeBlock
            language="bash"
            title="Signals and kill"
            code={`# List all signals
kill -l
...
disown %1                   # detach job from shell`}
          ...
          <h3 className="subsection-title">Priority and Scheduling</h3>
          <CodeBlock
            language="bash"
            title="Nice values and CPU scheduling"
            code={`# nice values range from -20 (highest priority) to 19 (lowest)
...
chrt -r -p 50 1234    # set real-time priority (requires root)`}
          />

          <InfoBox type="exam" title="Exam: Process Management with Ansible">
            <p>
              The <code>command</code> module runs commands without shell interpretation.
              The <code>shell</code> module uses <code>/bin/sh</code> and supports pipes, redirects,
              and variables. Use <code>command</code> when you don't need those features — it's safer
              and more predictable for exam scenarios.
            </p>
          </InfoBox>
        </div>
      )}
      ...
      {/* ===== FILESYSTEM HIERARCHY ===== */}
      {activeSection === 'Filesystem Hierarchy' && (
        <div>
          <h2 className="section-title">Filesystem Hierarchy Standard (FHS)</h2>
          <p>
            The FHS defines where files go in a Linux system. Ansible tasks that manage files
            need to know which directories require which permissions and context.
          </p>

          <div className="fhs-grid">
            {[
              { dir: '/', desc: 'Root of entire filesystem. Everything is under /. Only essential files at boot time.' },
              { dir: '/bin → /usr/bin', desc: 'Essential user command binaries: ls, cp, cat, bash. In RHEL 9, /bin is a symlink to /usr/bin.' },
              { dir: '/sbin → /usr/sbin', desc: 'System admin binaries: fdisk, mkfs, ip, firewalld. Symlinked to /usr/sbin in RHEL 9.' },
              { dir: '/etc', desc: 'Host-specific system configuration. Text files only. No binaries. This is what Ansible primarily modifies.' },
              { dir: '/var', desc: 'Variable data: logs (/var/log), spool (/var/spool), cache (/var/cache), runtime (/var/run).' },
              { dir: '/tmp', desc: 'Temporary files. World-writable with sticky bit. Cleared on reboot (tmpfs on modern systems).' },
              { dir: '/home', desc: 'User home directories. /home/username. Managed by Ansible\'s user module.' },
              { dir: '/root', desc: 'Home directory for root user. NOT under /home.' },
              { dir: '/boot', desc: 'Kernel images, initramfs, GRUB config. Do not modify unless you know what you are doing.' },
              { dir: '/dev', desc: 'Device files. Block devices (/dev/sda, /dev/nvme0n1), char devices, pseudo-devices (/dev/null, /dev/random).' },
              { dir: '/proc', desc: 'Virtual filesystem. Kernel and process info. Nothing on disk — kernel generates it.' },
              { dir: '/sys', desc: 'Virtual filesystem. Hardware topology, kernel objects. Used by udev for device management.' },
              { dir: '/run', desc: 'Runtime data since last boot. PID files, sockets. tmpfs — cleared at boot.' },
              { dir: '/usr', desc: 'Read-only user data: /usr/bin (binaries), /usr/lib (libraries), /usr/share (data), /usr/include (headers).' },
              { dir: '/lib → /usr/lib', desc: 'Essential shared libraries needed to boot. Symlinked in RHEL 9.' },
              { dir: '/opt', desc: 'Optional software packages. Third-party apps often install here.' },
              { dir: '/srv', desc: 'Data for services (web server files, FTP data). Often empty on RHEL.' },
              { dir: '/mnt', desc: 'Temporary mount point for sysadmin use.' },
              { dir: '/media', desc: 'Mount points for removable media (USB, CD).' },
            ].map(item => (
              <div key={item.dir} className="fhs-item">
                <code className="fhs-dir">{item.dir}</code>
                <p className="fhs-desc">{item.desc}</p>
              </div>
            ))}
          </div>

          <h3 className="subsection-title">Inodes — The True File Identity</h3>
          <p>
            An <strong>inode</strong> (index node) is a data structure on disk that stores all
            metadata about a file — except its name. The name is just an entry in a directory
            that points to an inode number. This explains why hard links work the way they do.
          </p>
          <div className="diagram-container">
            <pre>{`
...
find / -inum 48291`}
          ...
          <h3 className="subsection-title">Permissions Deep Dive</h3>
          <CodeBlock
            language="bash"
            title="File permissions, SUID, SGID, sticky bit"
            code={`# Permission bits: owner | group | other
...
setfacl -d -m u:nginx:rX /var/www/html`}
      ...
      {/* ===== STORAGE INTERNALS ===== */}
      {activeSection === 'Storage Internals' && (
        <div>
          <h2 className="section-title">Storage Internals</h2>

          <h3 className="subsection-title">Block Devices and Partitioning</h3>
          <div className="diagram-container">
            <pre>{`
...
`}</pre>
          ...
          <CodeBlock
            language="bash"
            title="Block device and partition management"
...
mount -a                        # mount everything in fstab (test!)`}
          ...
          <h3 className="subsection-title">LVM — Logical Volume Manager Deep Dive</h3>
          <div className="diagram-container">
            <pre>{`
  LVM Architecture:
...
`}</pre>
          ...
          <CodeBlock
            ...
            code={`# Step 1: Create Physical Volumes
...
pvremove /dev/sdb /dev/sdc`}
          ...
          <InfoBox type="warning" title="XFS vs ext4: Key Differences">
            <p>
              <strong>XFS:</strong> Default filesystem on RHEL 7+. Excellent for large files.
              Can be grown online (while mounted). <strong>Cannot be shrunk</strong>.
              Use <code>xfs_growfs</code> to extend.
            </p>
            <p style={{ marginTop: '8px' }}>
              <strong>ext4:</strong> Mature, reliable. Can be grown and shrunk (offline for shrink).
              Use <code>resize2fs</code>. Slightly slower than XFS for large sequential I/O.
            </p>
            <p style={{ marginTop: '8px' }}>
              <strong>Exam rule:</strong> When the exam says "XFS", never try to shrink it.
              Grow only. For ext4, you can shrink but you must unmount first.
            </p>
          </InfoBox>
        </div>
      )}
      ...
      {activeSection === 'Networking' && (
        <div>
          <h2 className="section-title">Linux Networking</h2>

          <h3 className="subsection-title">Interface Naming</h3>
          <p>
            RHEL uses <strong>predictable network interface names</strong> — no more eth0/eth1
            that could change on reboot. The naming is based on the physical location of the device.
          </p>
          <CodeBlock
            language="bash"
            title="Network interface commands"
            code={`# Interface naming conventions:
...
curl -v http://example.com      # HTTP test`}
          ...
          <h3 className="subsection-title">NetworkManager</h3>
          <CodeBlock
            language="bash"
            title="NetworkManager configuration"
            code={`# nmcli - NetworkManager command-line tool
...
nmcli connection modify "System ens3" \
...
ls /etc/NetworkManager/system-connections/`}
          ...
          <h3 className="subsection-title">firewalld — Zone-Based Firewall</h3>
          <CodeBlock
            language="bash"
            title="firewalld management"
            code={`# firewalld concepts:
...
firewall-cmd --add-masquerade --permanent`}
      ...
      {/* ===== SELINUX ===== */}
      {activeSection === 'SELinux' && (
        <div>
          <h2 className="section-title">SELinux — Security-Enhanced Linux</h2>
          <p>
            SELinux implements <strong>Mandatory Access Control (MAC)</strong> — as opposed to
            the traditional Unix <strong>Discretionary Access Control (DAC)</strong> (chmod/chown).
            With DAC, the file owner decides who can access it. With MAC, the kernel enforces
            policy regardless of file permissions. Even root cannot bypass SELinux policy.
          </p>
...
`}</pre>
          ...
          <CodeBlock
            language="bash"
            title="SELinux commands — the exam essentials"
...
reboot                            # trigger relabeling`}
          />

          <InfoBox type="exam" title="SELinux Exam Patterns">
            <p>
              The most common exam SELinux task: "A web server cannot serve files from
              /custom/webroot. Fix it." The solution is:
            </p>
            <ol style={{ marginTop: '8px' }}>
              <li><code>semanage fcontext -a -t httpd_sys_content_t "/custom/webroot(/.*)?"</code></li>
              <li><code>restorecon -Rv /custom/webroot/</code></li>
            </ol>
            <p style={{ marginTop: '8px' }}>
              Always use <code>semanage + restorecon</code>, not <code>chcon</code> alone —
              <code>chcon</code> changes are lost when the filesystem is relabeled.
            </p>
          </InfoBox>
        </div>
      )}
      ...
      {/* ===== SYSTEMD ===== */}
      {activeSection === 'Systemd' && (
        <div>
          <h2 className="section-title">Systemd — System and Service Manager</h2>
          <p>
            Systemd is PID 1 — the first process started by the kernel. It manages the entire
            system lifecycle: startup, services, timers, mounts, and shutdown. Understanding
            systemd unit types helps you write better Ansible playbooks.
          </p>

          <h3 className="subsection-title">Unit Types</h3>
          <div className="unit-grid">
            {[
              { type: '.service', desc: 'System services (httpd, sshd, nginx). Most common type.', color: 'var(--accent-primary)' },
              { type: '.socket', desc: 'Socket activation — service starts on first connection.', color: 'var(--accent-sky)' },
              { type: '.target', desc: 'Group of units — like runlevels. multi-user.target, graphical.target.', color: 'var(--success)' },
              { type: '.mount', desc: 'Mount points managed by systemd (from fstab or unit files).', color: 'var(--warning)' },
              { type: '.timer', desc: 'Cron replacement — schedule tasks with systemd timers.', color: 'var(--purple)' },
              { type: '.path', desc: 'Watch filesystem paths — trigger services on file changes.', color: 'var(--danger)' },
            ].map(u => (
              <div key={u.type} className="unit-card" style={{ borderLeftColor: u.color }}>
                <code style={{ color: u.color, fontSize: '0.85rem', fontWeight: 700 }}>{u.type}</code>
                <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginTop: '4px', marginBottom: 0 }}>{u.desc}</p>
              </div>
            ))}
          </div>

          <h3 className="subsection-title">Target Hierarchy (Boot Process)</h3>
          <div className="diagram-container">
            <pre>{`
...
`}</pre>
          ...
          <CodeBlock
            ...
            code={`# Service management (the essentials)
...
journalctl --disk-usage          # check log disk usage`}
          ...
          <InfoBox type="exam" title="Key Ansible → Systemd Relationship">
            <p>
              When you use the Ansible <code>service</code> or <code>systemd</code> module with
              <code>enabled: true</code>, Ansible runs <code>systemctl enable</code> behind the
              scenes. The exam requires services to survive reboot, so always set
              <code>enabled: true</code> AND <code>state: started</code>.
            </p>
          </InfoBox>
        </div>
      )}
